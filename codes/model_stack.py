...
def model_list(grid=False, seed=123):
   ...
   # xgbr_param = {'colsample_bytree': 0.7, 'learning_rate': 0.03, 'max_depth': 3, 'min_child_weight': 4, 'n_estimators': 400, 'nthread': 4, 'objective': 'reg:squarederror', 'subsample': 0.7}
    ...
    mlpr_param =  {'hidden_layer_sizes':  [(25), (50), (75),(100), (25, 50), (50,75),(75,100),(50,100), (25, 50, 75),(50, 75, 100)],
                                         'learning_rate_init': [0.01, 0.03,0.05, 0.001, 0.003, 0.0001, 0.0003], 'activation': ['relu', 'tanh','identity','logistic'], 'solver': ['adam', 'lbfgs','sgd'], 'learning_rate' : ['constant', 'adaptive', 'invscaling']}

    # MLPR_ = MLPRegressor(activation = 'relu', solver = 'adam', max_iter=20000, random_state=seed, early_stopping=False, verbose=0,)
    MLPR_ = MLPRegressor( max_iter=20000, random_state=seed, early_stopping=False, verbose=0,)
    ...
    if grid ==True:
        ...
        model_stack = {"MLPR":[MLPR_, mlpr_param], "XBGR":[XGBR_, xgbr_param], "LGBMR": [LGBMR_, lgbmr_param],"CATBOOST":[CATBOOST_,catboost_param]}
